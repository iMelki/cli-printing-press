...
func TestClassifyWithBaseTreatsPureTemplateDriftAsClean(t *testing.T) {
	t.Parallel()

	baseDir := t.TempDir()
	pubDir := t.TempDir()
	freshDir := t.TempDir()
	rel := filepath.Join("internal", "cli", "sync.go")
	oldEmission := `// Generated by CLI Printing Press. DO NOT EDIT.
package cli

const syncWarningJSON = "{\"warning\":\"unsafe\"}"
`
	freshEmission := `// Generated by CLI Printing Press. DO NOT EDIT.
package cli

const syncWarningJSON = "{\"warning\":\"safe\"}"
`
	for _, root := range []string{baseDir, pubDir, freshDir} {
		require.NoError(t, os.MkdirAll(filepath.Join(root, filepath.Dir(rel)), 0o755))
	}
	require.NoError(t, os.WriteFile(filepath.Join(baseDir, rel), []byte(oldEmission), 0o644))
	require.NoError(t, os.WriteFile(filepath.Join(pubDir, rel), []byte(oldEmission), 0o644))
	require.NoError(t, os.WriteFile(filepath.Join(freshDir, rel), []byte(freshEmission), 0o644))

	report, err := Classify(pubDir, freshDir, Options{Force: true, BaseDir: baseDir})
	require.NoError(t, err)

	assert.Equal(t, VerdictTemplatedClean, verdictMap(report)[filepath.ToSlash(rel)],
		"snapshot identical to original emission is version drift only; fresh must win")
}
